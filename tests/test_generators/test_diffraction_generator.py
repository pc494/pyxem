...
class TestDiffractionCalculator:
    ...
    def test_appropriate_scaling(
            self, diffraction_calculator: DiffractionGenerator):
    ...
    def test_appropriate_intensities(
            self, diffraction_calculator, local_structure):
        ...
        smaller = np.greater_equal(
            diffraction.intensities[central_beam],
            diffraction.intensities)
        assert np.all(smaller)

    def test_calculate_profile_class(
            self, local_structure, diffraction_calculator):
        ...
        atom = diffpy.structure.atom.Atom(
            atype='Ni', xyz=[0, 0, 0], lattice=latt)
        hexagonal_structure = diffpy.structure.Structure(
            atoms=[atom], lattice=latt)
