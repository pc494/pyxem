...
def create_spot():
    ...
    dp = pxm.ElectronDiffraction(np.asarray(
        [[z, z], [z, z]]))  # this needs to be in 2x2
    return dp


@pytest.mark.filterwarnings('ignore::UserWarning')  # various skimage warnings
def test_conventional_xc(diffraction_pattern):
    SPR_generator = SubpixelrefinementGenerator(
        diffraction_pattern, np.asarray([[1, -1]]))
...
def test_assertioned_xc(create_spot):
    spr = SubpixelrefinementGenerator(
        create_spot, np.asarray([[90 - 64, 30 - 64]]))
